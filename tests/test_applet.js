...
applet._render();
assert.equal(applet.label, 'Codex 5h 73% · W 41%');
...
limits.primary = null;
applet._render();
assert.equal(applet.label, 'Codex 5h — · W 41%');
...
assert.equal(applet.label, 'Codex 1K today');
...
applet._data = null;
applet._render();
assert.equal(applet.label, 'Codex — today');
applet.settings.change('display-mode', 'limits');
assert.equal(applet.label, 'Codex 5h — · W —');
applet._refresh();
